...
RT-Thread Device Profile adapters use one common deferred worker by default. The hidden
`PKG_CANOPENNODE_PROFILE_RTT_SHARED_WORKER` symbol is derived automatically whenever at least one enabled Profile adapter has not requested a dedicated worker.

| Option | Default | Purpose |
|---|---:|---|
| `PKG_CANOPENNODE_PROFILE_THREAD_STACK_SIZE` | `2048` | Stack for the common `co_prof` worker; size for the deepest enabled shared Profile pass. |
| `PKG_CANOPENNODE_PROFILE_THREAD_PRIORITY` | `5` | Priority of `co_prof`; its numeric value must be greater than `PKG_CANOPENNODE_RT_THREAD_PRIORITY`. |
| `PKG_CANOPENNODE_CIA401_DEVICE_RTT_DEDICATED_WORKER` | `n` | Replace CiA 401 participation in `co_prof` with a private `co_401` worker. |
| `PKG_CANOPENNODE_CIA402_DEVICE_RTT_DEDICATED_WORKER` | `n` | Replace CiA 402 participation in `co_prof` with a private `co_402` worker. |
| `PKG_CANOPENNODE_CIA401_THREAD_STACK_SIZE` | `1536` | `co_401` stack; visible only in dedicated-worker mode. |
| `PKG_CANOPENNODE_CIA401_THREAD_PRIORITY` | `6` | `co_401` priority; visible only in dedicated-worker mode. |
| `PKG_CANOPENNODE_CIA402_THREAD_STACK_SIZE` | `2048` | `co_402` stack; visible only in dedicated-worker mode. |
| `PKG_CANOPENNODE_CIA402_THREAD_PRIORITY` | `5` | `co_402` priority; visible only in dedicated-worker mode. |
...
The realtime timer wakes the common `co_prof` worker once per deferred cycle; all Profiles assigned to it run sequentially in lifecycle registration order while one communication generation is pinned. Select a dedicated worker only when a Profile needs independent priority or isolation from another Profile's WCET/blocking risk. `PKG_CANOPENNODE_CIA402_DEMO_SYNC_LOG` selects the dedicated CiA 402 worker so ULOG remains outside the shared lifecycle lock. Automatic construction remains opt-in.
