...
本文说明阶段 A3 的实现边界和运行时契约。A3 只建立多轴 Device 的 Pure-C PDS 核心，不引入 RT-Thread、STM32/BSP 或运行时 OD 生成逻辑。

## 1. A3 做什么

A3 把已经由 XDD 生成的 CiA 402 对象绑定到本地多轴运行时，并为每个 axis 提供独立 PDS supervisor：
...
- 所有 manager/axis/config/DriveIF storage 均由应用持有，A3 不分配 heap。

![CiA 402 Pure-C Device core](../assets/cia402-device-core.svg)

A3 不实现 RT-Thread worker、STM32 HAL、PWM/ADC/FOC、PP/PV/HM/CSP/CSV/CST mode runtime，也不修改 `CANopenNode/` core。
...
XDD/生成后的 `OD.c`、`OD.h` 是对象存在性和语义数据类型的 source of truth。A3 运行时不会创建隐藏对象，只验证 CANopenNode runtime 可以看到的结构和属性。

每个配置 axis 当前要求以下对象：

| Axis0 index | 含义 | 长度 | A3 要求的 access/mapping |
|---:|---|---:|---|
...
3. byte length 与表中一致；
4. 全部 runtime-visible attribute 与 A3 contract 一致，包括拒绝未声明的 SRDO mapping 和 `ODA_STR`；
...
> CANopenNode 当前 runtime OD metadata 不保存 XDD 中完整的语义 data-type 信息，例如有符号/无符号类型名称。A3 因此只能在 runtime 验证 object code、sub-index 结构、byte width 和 access/mapping；INTEGER/UNSIGNED 等语义类型仍由 XDD/生成物负责。这不是 CiA 402 conformance 声明。
...
`configs[*].drive` 和 `configs[*].driveObject` 同样必须保持有效。A3 不调用 `malloc/calloc/realloc/free`，也不持有 RT-Thread object。
